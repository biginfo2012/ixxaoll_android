...
import { AppScreen, AppText, AppButton, AppStatusIcon, AppPicker, ActivityIndicator, AppFileUpload } from "app/components";
...
const EventScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      controller.abort();
    }; //TODO: do this in other screen. this handles unmounted component memory leak
  }, []);
  ...
  const save = async () => {
    ...
    if (hasFileUpload) {
      ...
      // const response = await tempFileUpload(filePropertyBlock.item[1].uri, filePropertyBlock.item[1].value);
